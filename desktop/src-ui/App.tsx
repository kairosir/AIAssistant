...
const isDocumentSearchRequest = (query: string) => /(найд|поиск|ищу|покажи|в каких файлах|где встреча|где упомина|по документ|в документ|сводк|резюм|обобщ|суммар|сравн|проанализируй (?:файл|документ)|что (?:в|написано в) (?:этом )?(?:файле|документе)|сколько .*?(?:файл|документ)|search|find|which files|in my documents?|summar|compare|analy[sz]e (?:the )?(?:file|document)|how many .*?(?:files|documents))/iu.test(query);
...
function ChatView({ thread, threads, indexStatus, aiStatus, onThreadChange, onIndexStatus }: { thread: ChatThread; threads: ChatThread[]; indexStatus: IndexStatus; aiStatus: AiStatus; onThreadChange: (thread: ChatThread) => void; onIndexStatus: (status: IndexStatus) => void }) {
  ...
  return <div className="workspace">
    <main className="chat-main">
      <header className="chat-header" data-tauri-drag-region><div><h1>{thread.title}</h1><p title={aiStatus.provider || "officeghost-local-core"}><StatusDot active />OfficeGhost Local AI · встроен</p></div><button className="context-button"><Files size={16} />Документы — по запросу</button></header>
...
function LibraryView({ indexStatus, settings, onIndexStatus, onSettings }: { indexStatus: IndexStatus; settings: Settings; onIndexStatus: (status: IndexStatus) => void; onSettings: (settings: Settings) => void }) {
  ...
  const hasRoots = !!settings.roots?.length;
  const chooseFolder = async () => { const next = await chooseIndexFolder(); onSettings(next); if (next.roots?.length) onIndexStatus(await refreshIndex()); };
  ...
  useEffect(() => { if (hasRoots) searchDocuments("").then(setResults); else setResults([]); }, [hasRoots]);
  return <main className="page-main library-page">
    <header className="page-header" data-tauri-drag-region><div><span className="eyebrow">Локальная библиотека</span><h1>Ваши документы</h1><p>OfficeGhost индексирует только те папки, которые вы выбрали сами.</p></div><div className="header-actions"><button className="secondary-button" onClick={chooseFolder}><FolderOpen size={16} />{hasRoots ? "Добавить папку" : "Выбрать папку"}</button>{hasRoots && <button className="primary-button" onClick={async () => onIndexStatus(await refreshIndex())}><RefreshCw className={indexStatus.state === "indexing" ? "spin" : ""} size={16} />Обновить индекс</button>}</div></header>
    ...
    {!hasRoots ? <section className="folder-onboarding"><div className="folder-onboarding-icon"><FolderOpen size={28} /></div><span className="eyebrow">Первый запуск</span><h2>Выберите папку с документами</h2><p>OfficeGhost не просматривает весь компьютер. Доступ будет только к выбранной вами папке, а macOS запомнит ваше разрешение.</p><button className="primary-button" onClick={chooseFolder}><FolderOpen size={16} />Выбрать папку</button><div className="folder-privacy-note"><ShieldCheck size={16} /><span><strong>Вы контролируете доступ</strong><small>Позже можно добавить или удалить папки в Библиотеке.</small></span></div></section> : <><section className="stats-row"><div className="stat-card"><div><Files size={18} /></div><span><strong>{(indexStatus.fileCount || indexStatus.total || 0).toLocaleString("ru-RU")}</strong><small>Документов в индексе</small></span></div><div className="stat-card"><div><Check size={18} /></div><span><strong>11 форматов</strong><small>PDF, Office, текст и таблицы</small></span></div><div className="stat-card"><div><Clock3 size={18} /></div><span><strong>Автоматически</strong><small>Выбранные папки сохранены</small></span></div></section>
  ...
  return <main className="account-view">
    ...
    <div className="account-app-content">
      ...
      <section className="account-app-grid">
        ...
        <article><div className="account-metric-icon"><MonitorCheck size={19} /></div><span><small>ЭТО УСТРОЙСТВО</small><strong>OfficeGhost 0.3.4</strong><p>{updateStatus.available ? `Доступно обновление ${updateStatus.version}` : "Приложение подключено к аккаунту."}</p></span></article>
      </section>
